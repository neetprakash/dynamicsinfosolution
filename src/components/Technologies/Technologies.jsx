...
import { Tabs, TabsHeader, Tab, TabsBody, TabPanel, Accordion, AccordionHeader, AccordionBody } from '@material-tailwind/react';
  ...
  return (
    <section className='p-3 lg:p-20'>
      ...
      <div className='md:hidden'>
        {data.map(({ label, value, icon }, index) => (
          <Accordion open={open === index}>
            <AccordionHeader onClick={() => handleOpen(index)}><div className="flex items-center gap-4 text-xl text-black">
              {React.createElement(icon, { className: "w-5 h-5" })}
              {label}
            </div></AccordionHeader>
            {data.map(({ value, desc }) => (
              <AccordionBody  className="py-0  flex flex-wrap justify-start gap-4">
                {desc.map((tech, index) => (
      ...
      <div className='hidden md:block'>
        <Tabs value="web_dev" orientation="vertical">
